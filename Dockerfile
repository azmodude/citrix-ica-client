...
RUN dnf install -y --setopt=install_weak_deps=False \
  /host/assets/ICAClient-rhel-${ICACLIENT_VERSION}.x86_64.rpm \
  openssl libsecret pulseaudio pulseaudio-utils nss && \
  dnf clean all

COPY assets/certs/*.pem /opt/Citrix/ICAClient/keystore/cacerts
RUN openssl rehash /opt/Citrix/ICAClient/keystore/cacerts/

RUN mkdir /home/citrix/.ICAClient
COPY assets/config/*.ini /home/citrix/.ICAClient
